...
function createAsyncData(nuxtApp, key, _handler, options, initialCachedData) {
  var _a, _b
  ;(_b = (_a = nuxtApp.payload._errors)[key]) != null
    ? _b
    : (_a[key] = asyncDataDefaults.errorValue)
        ...
        const cachedData =
          'cachedData' in opts
            ...
            : options.getCachedData(key, nuxtApp, {
                cause: (_b2 = opts.cause) != null ? _b2 : 'refresh:manual',
              })
        if (cachedData != null) {
...
const useCourses = () => {
  const list = async () => {
    const { data, error } = await useFetch('/api/courses', '$xdzWn17RwL')
    return { data, error }
  }
  const getById = async (id) => {
    const { data, error } = await useFetch(`/api/courses-${id}`, '$lHign-pojF')
    return { data, error }
  }
  return { list, getById }
}
...
//# sourceMappingURL=useCourses-BYQwKJHx.mjs.map
